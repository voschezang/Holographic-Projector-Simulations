...
inline void superposition_per_block_naive_helper(const dim3 gridDim, const dim3 blockDim, cudaStream_t stream,
                                                 ...
                                                 WAVE *d_y_block, const bool append_result = false)
...
inline std::vector<WAVE> transform_2D_batches(const std::vector<WAVE> &x,
  ...
  size_t batch_out_size = batch_size.x * batch_size.y; // TODO use dim3?
    ...
    for (size_t n = 0; n < n_batches.x; ++n) {
      ...
      for (size_t i_stream = 0; i_stream < p.n_streams; ++i_stream) {
        ...
        if (m < n_batches.y) {
          ...
          if (n == 0) {
            // cp y data with index m, with stream s (x batch data has already been copied)
...
std::vector<WAVE> time_transform(const std::vector<WAVE> &x,
  ...
  // auto y = transform_2D_batches<direction, Algorithm::Naive, shared_memory>(x, u, v, p);
  auto y = transform_2D_batches<direction, Algorithm::Alt, shared_memory>(x, u, v, p);
